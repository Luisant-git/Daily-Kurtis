...
export default function Checkout() {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-neutral-600 mt-3">
          Thank you for choosing Daily Kurtis. Your order <span className="font-medium text-[#800000]">#DK{Math.floor(Math.random() * 90000) + 10000}</span> has been placed successfully.
        </p>
  ...
  const onSubmit = (data) => {
    console.log("Order:", { data, payment, items });
    clearCart();
    setPlaced(true);
      ...
      <div className="mt-6 grid lg:grid-cols-[1fr_400px] gap-8">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
          ...
          <Step n="01" title="Shipping Address" sub="Where should we deliver your order?">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <Field label="Phone" error={errors.phone?.message}>
                <input className="input" {...register("phone", { required: "Required" })} placeholder="+91 98765 43210" />
              </Field>
              <Field label="Street Address" full error={errors.address?.message}>
                <input className="input" {...register("address", { required: "Required" })} placeholder="Flat, Apartment, Street" />
              </Field>
          ...
          <Step n="02" title="Payment Method" sub="Select your preferred way to pay.">
            ...
            <div className="mt-5 flex items-start gap-2 text-xs text-neutral-600 bg-[#FFF8F8] p-4 rounded-xl">
              ...
              <p>Your data is encrypted using industry-standard SSL. We never store your card details.</p>
            </div>
          </Step>
        ...
        <aside className="bg-white border border-[#E9E5E5] rounded-2xl p-6 h-fit sticky top-28">
          ...
          <div className="mt-5 space-y-3 max-h-80 overflow-y-auto pr-2">
            {items.map((i) => (
              <div key={`${i.product.id}-${i.size}-${i.color}`} className="flex gap-3">
                <img src={i.product.images[0]} className="w-16 aspect-[4/5] rounded-lg object-cover" alt="" />
                ...
                <p className="text-sm font-semibold text-[#800000]">{formatINR(i.product.discountPrice * i.quantity)}</p>
              </div>
            ))}
          </div>
          ...
          <div className="border-t border-[#E9E5E5] mt-5 pt-5 space-y-2 text-sm">
            ...
            <div className="flex justify-between"><span>Shipping</span><span className="text-[#16A34A]">Free</span></div>
            <div className="flex justify-between"><span>GST (5%)</span><span>{formatINR(tax)}</span></div>
          </div>
